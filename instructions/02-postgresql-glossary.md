# 02 - PostgreSQL Glossary Baseline

Glossary là baseline, không phải whitelist. Giữ English nếu technical term chưa có trong bảng nhưng đó là cách viết tự nhiên và chính xác hơn.
...
## Identifiers và casing

Giữ nguyên chính tả/casing của:

- SQL keyword trong code;
- function/operator/type name;
- catalog name;
...
`pg_catalog`, `information_schema`, `pg_stat_activity`, `current_setting`, `generate_series`, `jsonb`, `SERIAL`, `BIGSERIAL`, `EXPLAIN`, `VACUUM`, `ANALYZE`, `psql`, `postgresql.conf`.

Không global-replace glossary một cách mù quáng. Cùng một English word có thể mang nghĩa thường ở context khác.
