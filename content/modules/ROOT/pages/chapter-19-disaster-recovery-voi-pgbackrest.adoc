...
. Trước hết, hãy tạo một ssh key cho Postgres user trên cả hai server. Với tư cách PostgreSQL user, hãy thực thi:
...
[%start=2]
. Bây giờ, trên cả hai server, trong directory `~/.ssh` của postgres user sẽ có hai file:
...
. Cách nhanh nhất để exchange public key giữa hai server là sử dụng command `ssh-copy-id`:
